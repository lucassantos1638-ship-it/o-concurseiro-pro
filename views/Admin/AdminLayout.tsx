...
import AdminImportMaterias from './AdminImportMaterias';
      ...
      <header className="h-16 bg-slate-900 text-white flex items-center justify-between px-6 shrink-0 shadow-lg">
        <div className="flex items-center gap-6 overflow-x-auto hide-scrollbar">
          ...
          <nav className="flex gap-2">
            {['concursos', 'cargos', 'materias', 'materias-import', 'questoes', 'importar', 'importar-pdf', 'webhooks', 'backup'].map((tab) => (
              <button
                key={tab}
              ...
              >
                {tab === 'importar' ? 'Excel Import' :
                  tab === 'importar-pdf' ? 'IA Import (PDF)' :
                    tab === 'materias-import' ? 'Excel Matérias' :
                      tab}
              </button>
            ))}
          </nav>
