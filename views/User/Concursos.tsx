                 ...
                 <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                    ...
                    <div className="mt-6 w-full max-w-[280px]">
                        <button 
                          ...
                          className={`w-full py-3.5 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all shadow-xl flex items-center justify-center gap-3 ${
                            isMyCargo 
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden mb-8">
            <div className="relative h-[240px] md:h-[280px] w-full rounded-[32px] overflow-hidden m-2 md:m-3">
               ...
               <div className="absolute bottom-6 left-8 flex items-end justify-between right-8">
                  <div className="flex flex-col gap-2 md:gap-3">
                    <span className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-widest text-white shadow-lg ${
                      status === 'Edital Publicado' || status === 'Inscrições Abertas' ? 'bg-primary' : 'bg-amber-500'
                    }`}>
                      {status}
                    </span>
            ...
            <div className="px-4 md:px-8 pb-8 pt-6">
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
                  ...
                  <a href={selectedConcurso.links.editalPdf} target="_blank" rel="noopener noreferrer" className={`flex items-center justify-center gap-2 p-3 rounded-xl font-bold text-[10px] uppercase tracking-tighter transition-all ${selectedConcurso.links.editalPdf ? 'bg-slate-50 text-slate-600 hover:bg-slate-200' : 'bg-slate-50 text-slate-300 pointer-events-none'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-12">
             {cargosDoConcurso.map(cargo => (
               <div key={cargo.id} className="bg-white p-6 md:p-7 rounded-[32px] border border-slate-100 flex flex-col justify-between hover:border-primary transition-all shadow-sm group">
                  <div className="mb-6">
                    <div className="flex justify-between items-start gap-2 mb-3">
                      <h4 className="font-bold text-slate-800 text-base md:text-xl group-hover:text-primary transition-colors uppercase leading-tight">{cargo.nome}</h4>
                      <span className="bg-slate-100 text-slate-500 text-[8px] font-black px-2 py-1 rounded-full uppercase border shrink-0">Nível {cargo.nivel}</span>
                    </div>
                    <div className="flex items-center gap-6">
                       <div className="flex flex-col">
                          <span className="text-[8px] font-black text-slate-400 uppercase">Salário</span>
                          <span className="font-bold text-emerald-600 text-xs md:text-sm">{cargo.salario || 'A definir'}</span>
                       </div>
                       <div className="flex flex-col">
                          <span className="text-[8px] font-black text-slate-400 uppercase">Vagas Totais</span>
                          <span className="font-bold text-slate-700 text-xs md:text-sm">{cargo.totalVagas}</span>
                       </div>
                    </div>
                  </div>
                  <button onClick={() => setViewingCargoId(cargo.id)} className="w-full bg-slate-50 text-slate-600 font-black py-4 rounded-2xl text-[10px] uppercase tracking-widest hover:bg-primary hover:text-white transition-all group-hover:shadow-lg group-hover:shadow-primary/10">Saiba Mais</button>
               </div>
             ))}
             {cargosDoConcurso.length === 0 && (
                <div className="col-span-2 bg-white p-12 rounded-[32px] border border-dashed text-center">
